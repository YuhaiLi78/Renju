...
def main() -> None:
    ...
    while True:
        ...
        if game.ruleset == RuleSet.RENJU and game.swap_available and not game.swap_decided:
            swap_input = input("White may swap colors. Swap? (y/n) > ").strip().lower()
            swap = swap_input in {"y", "yes"}
            print(game.decide_swap(swap))
            continue

        if game.ruleset == RuleSet.RENJU and game.candidate_removal_required:
        ...
        candidate_note = ""
        if game.ruleset == RuleSet.RENJU and (game.should_start_candidate_phase() or game.candidate_points):
